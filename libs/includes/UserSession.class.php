...
/**
     * Authorized function have has 4 level of checks
     * Check that the IP and user agent field is filled.
     * check if the session is correct and active
     * Check that the current IP is the same as the previous IP
     * Check that the current user agent is the same as the previous user agent
     *
...
public static function authorize($token)
{
    try{
        ...
        {
            if($session->isVaild() and $session->isActive())
            {
    ...
    return false;
     ...
     /**
      * Check if the validate of the session is within one hour, else it inactive.
      *
      * @return boolean
      */
      ...
      public function isValid()
      {
        if (isset($this->data['login_time'])) {
            $login_time = DateTime::createFormFormat('Y-m-d H:i:s', $this->data['login_time']);
    ...
    //This funcction remove current seesion
